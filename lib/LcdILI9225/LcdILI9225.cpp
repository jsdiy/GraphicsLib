...
//コンストラクタ
LcdILI9225::LcdILI9225()
{
	using	namespace	ILI9225Config;
	if (Controller == EController::ILI9225) { screenWidth = LcdWidth = 176;	screenHeight = LcdHeight = 220; }
	//if (Controller == EController::ILI9341) { screenWidth = LcdWidth = 240;	screenHeight = LcdHeight = 320; }

	grBuffer = nullptr;
}

//初期化
void	LcdILI9225::Initialize()
{
	using	namespace	ILI9225Config;
	...
	//SPIデバイス情報を初期化する
	glcd.Initialize(SpiMode, SpiClock, /*PinSS*/GPIO_NUM_NC, PinDC);
	...
	//画面系
	lcdReg.R01DriverOutputControl = BitVal::DriverSS | 0x001C;	//SM-GS=00, NL[4:0]=11100(528[176xRGB]x220)
	lcdReg.R03EntryMode = BitVal::EntryBGR | 0x0030;	//{I/D[1:0]=11, AM=0}:標準の向き（回転/反転なし）
	SendRegVal(ERegIdx::IdxDRIVER, lcdReg.R01DriverOutputControl);
	SendRegVal(ERegIdx::IdxENTRY, lcdReg.R03EntryMode);
	...
	*/
	...
	//画面表示オン
	lcdReg.R07DisplayControl1 = BitVal::DisplayREV | 0x0013;	//{GON=1, D[1:0]=11}:表示オン	※{=0, =00}で表示オフ
	SendRegVal(ERegIdx::IdxDISPLAY, lcdReg.R07DisplayControl1);
...
//ハードウェアリセット
//データシート	14.4. Reset Timing Characteristics
//データシート	13.4. Power Supply Configuration
void	LcdILI9225::HwReset()
{
	using	namespace	ILI9225Config;
	if (PinRESET == GPIO_NUM_NC) { return; }
	pinMode(PinRESET, OUTPUT);
	digitalWrite(PinRESET, LOW);	delay(10);	//1m以上
	digitalWrite(PinRESET, HIGH);	delay(100);	//51ms以上
}

//レジスタ値を送信する
void	LcdILI9225::SendRegVal(ERegIdx regIdx, uint16_t regVal)
{
	SpiTxStart();
	glcd.SendCommand((uint8_t)regIdx);
	glcd.SendData(regVal);
	SpiTxEnd();
}

//GRAMデータを送信する
void	LcdILI9225::SendGRamData(uint8_t* datas, size_t length)
{
	SpiTxStart();
	glcd.SendData(datas, length);
	SpiTxEnd();
}
...
void	LcdILI9225::RotateFlip(ERotFlip param)
{
	...
	lcdReg.R03EntryMode &= ~BitVal::EntryAMID_BitMask;
	lcdReg.R03EntryMode |= val;
	glcd.BeginTransaction();
	SendRegVal(ERegIdx::IdxENTRY, lcdReg.R03EntryMode);
	glcd.EndTransaction();

	//回転状態に応じて画面の縦横サイズを入れ替える
	if (val & BitVal::EntryAM_dirV) { screenWidth = LcdHeight;	screenHeight = LcdWidth; }
...
void	LcdILI9225::SetGRamArea(int16_t x, int16_t y, int16_t w, int16_t h)
{
	...
	uint16_t val = lcdReg.R03EntryMode & BitVal::EntryAMID_BitMask;
	switch (val)
	{
	//画面縦長グループ
	default:
...
void	LcdILI9225::FillRect(int16_t x, int16_t y, int16_t w, int16_t h, const Color& color)
{
	...
	SetGRamArea(x, y, w, h);
	SpiTxStart();
	glcd.SendCommand((uint8_t)ERegIdx::IdxWRGRAM); 
...
void	LcdILI9225::DrawImage(int16_t x, int16_t y, int16_t w, int16_t h, const uint8_t* imgData, size_t dataLength)
{
	glcd.BeginTransaction();
	SetGRamArea(x, y, w, h);
	SpiTxStart();
...
//画面全体を塗りつぶす
void	LcdILI9225::ClearScreen(const Color& color)
{
	FillRect(0, 0, screenWidth, screenHeight, color);
}

//外部のオブジェクトへバッファを提供（共有）する
uint8_t*	LcdILI9225::GetLineBuffer(size_t* bufLength)
{
